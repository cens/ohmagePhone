...
import edu.ucla.cens.andwellness.AndWellnessApi.ImageReadResponse;
...
import edu.ucla.cens.andwellness.feedback.utils.ISO8601Utilities;
...
public class FeedbackService extends WakefulIntentService {
	...
	protected void doWakefulWork(Intent intent) {
		...
		// check the lock; if it's locked, exit
		if (workingLock.tryLock()) {
			try {
				...
				// and now that we're done, release the lock
				workingLock.unlock();
			}
		}
		else
		{
			// it must already be busy; return?
			Log.v(TAG, "Service already busy, exiting");
		}
	}
	...
	public static boolean ensurePhotoExists(Context context, String campaignUrn, String photoUUID) {
		...
		if (!photo.exists()) {
			...
			// and then do so!
			AndWellnessApi api = new AndWellnessApi(context);
