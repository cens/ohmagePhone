...
public class ClassifierService extends WakefulIntentService
{
	...
	private Vector<ArrayList<Double>> getAccSamples()
	{
		try
		{
			if (mAccel == null)
			{
				...
				Log.e(TAG, "mAccel fails to not be null even though onbind returned. Losing sample.");
				return null;
			}
			...
			// }
			Log.d(TAG, (System.currentTimeMillis() - mAccel.getLastTimeStamp()) / 1000 + " is how old this sample is!!!!!!");
			Vector<ArrayList<Double>> curList = new Vector<ArrayList<Double>>();
			curList.add((ArrayList<Double>) mAccel.getLastXValues());
			curList.add((ArrayList<Double>) mAccel.getLastYValues());
			curList.add((ArrayList<Double>) mAccel.getLastZValues());
			curList.add((ArrayList<Double>) mAccel.getLastForce());
	...
	private void getTransportMode()
	{
		...
		try
		{
			...
			if (mWiFiGPS != null)
			{
				loc = mWiFiGPS.getLocation();
				wifiData = mWiFiGPS.getWiFiScan();
				// globalLoc = mWiFiGPS.getLocation();
				if (!setInterval)
				{
					mWiFiGPS.suggestInterval(Mobility.SERVICE_TAG, (int) Mobility.sampleRate);
					setInterval = true;
				}
	...
	protected void doWakefulWork(Intent intent)
	{
		...
		bindService(new Intent(edu.ucla.cens.wifigpslocation.IWiFiGPSLocationService.class.getName()), mConnection, Context.BIND_AUTO_CREATE);
		bindService(new Intent(IAccelService.class.getName()), AccelServiceConnection, Context.BIND_AUTO_CREATE);
