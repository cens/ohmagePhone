...
public class OldUploadService extends WakefulIntentService{
	...
	private void uploadSurveyResponses() {
		...
		for (Campaign campaign : dbHelper.getCampaigns()) {
			...
			if (responseRows.size() > 0) {
				...
				OhmageApi.UploadResponse response = mApi.surveyUpload(serverUrl, username, hashedPassword, SharedPreferencesHelper.CLIENT_STRING, campaign.mUrn, campaign.mCreationTimestamp, responsesJsonArray.toString());
	...
	private void handleErrors(OhmageApi.UploadResponse response, Campaign problematicCampaign) {
